...
while (True):
    ...
    if (nowControlTime - lastControlTime > .01):
        ...
        final_position = [11, 11]
        threshold = 0.7
        maxForce = 20
        targetVelocity = 15
        sensor_results = [results[_][3] for _ in range(100)]

        if totalHits(sensor_results) <= 0:
            value = np.array([final_position[0] - carPos[0], final_position[1] - carPos[1]])
            steeringAngle = np.clip(math.atan2(value[1], value[0]) - p.getEulerFromQuaternion(carOrn)[2], -0.5, 0.5)
        else:
            distance = min([math.dist(carPos[:2], i) for i in [i[:2] for i in sensor_results if i[0] != 0.0 and i[1] != 0.0]])
            print(threshold, distance)
            if threshold > distance:
                if totalHits(sensor_results[:50]) > totalHits(sensor_results[50:]):
                    steeringAngle = -1
                else:
                    steeringAngle = 1
            else:
